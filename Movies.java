...
public class Movies implements IMovies{
    ...
    public HashMap<Integer, CollectionsObject> CollectionsHashMap;
    public SortedMovieArrayList MovieList;
    int count;
    ...
    public Movies(Stores stores) {
        ...
        this.CollectionsHashMap = new HashMap<>();
        count = 0;
        MovieList = new SortedMovieArrayList();
    }
    ...
    public boolean add(int id, String title, String originalTitle, String overview, String tagline, String status, Genre[] genres, LocalDate release, long budget, long revenue, String[] languages, String originalLanguage, double runtime, String homepage, boolean adult, boolean video, String poster) {
        if (MovieHashMap.containsKey(id)) {
            return false;
        }
        MoviesObject newMovie = new MoviesObject(id, title, originalTitle, overview, tagline, status, genres, release, budget, revenue, languages, originalLanguage, runtime, homepage, adult, video, poster);
        MovieHashMap.add(id, newMovie);
    ...
    public boolean remove(int id) {
        ...
        MovieList.removeID(id);
        count --;
        return MovieHashMap.remove(id);
    ...
    public int[] getAllIDs() {
        DynamicArrayList<Integer> keys = MovieHashMap.getAllKeys();
    ...
    public int[] getAllIDsReleasedInRange(LocalDate start, LocalDate end) {
        DynamicArrayList<Integer> filteredKeys = new DynamicArrayList<>();

        for (int id : getAllIDs()) { 
            MoviesObject newMovie = MovieHashMap.get(id); 
            if (newMovie != null) {
                LocalDate releaseDate = newMovie.getRelease();
    ...
    return filteredKeys.toIntArray();
}
    ...
    public String getTitle(int id) {
        MoviesObject movie = MovieHashMap.get(id);
        if (movie == null) {
            return null;
        }
    ...
    public boolean setVote(int id, double voteAverage, int voteCount) {
        ...
        movie.setVote(voteAverage, voteCount);
        return true;
    }
    ...
    public boolean addToCollection(int filmID, int collectionID, String collectionName, String collectionPosterPath, String collectionBackdropPath) {
        MoviesObject movie = MovieHashMap.get(filmID);
        if (movie == null) {
            return false;
        }

        CollectionsObject collection = CollectionsHashMap.get(collectionID);
        if (collection == null) {
            collection = new CollectionsObject(collectionID, collectionName, collectionPosterPath, collectionBackdropPath);
            CollectionsHashMap.add(collectionID, collection);
        }
    ...
    public int[] getFilmsInCollection(int collectionID) {
        ...
        DynamicArrayList<Integer> filmIDs = new DynamicArrayList<>();
        DynamicArrayList<Integer> allMovieIDs = MovieHashMap.getAllKeys();
        for (int i = 0; i < allMovieIDs.size(); i++) {
            int movieID = allMovieIDs.get(i);
            MoviesObject movie = MovieHashMap.get(movieID);
            
            if (movie != null && movie.getCollectionID() == collectionID) {
                filmIDs.add(movieID);
            }
        }
        
        int[] result = new int[filmIDs.size()];
        for (int i = 0; i < filmIDs.size(); i++) {
            result[i] = filmIDs.get(i);
        }
    ...
    public Company[] getProductionCompanies(int id) {
        ...
        DynamicArrayList<Company> companyList = movie.getProductionCompanies();
    ...
    public String[] getProductionCountries(int id) {
        ...
        DynamicArrayList<String> countryList = movie.getProductionCountries();
    ...
    public int[] findFilms(String searchTerm) {
        DynamicArrayList<Integer> ListMovies = new DynamicArrayList<>();
        for(int i = 0; i < MovieList.size(); i ++){
            MoviesObject newMovie = MovieList.get(i);
            if (newMovie.getTitle().contains(searchTerm) || newMovie.getOriginalTitle().contains(searchTerm) || newMovie.getOverview().contains(searchTerm) ){
                ListMovies.add(newMovie.getID());
            }
        }
       return ListMovies.toIntArray();
    }
}
